...
export function RevenueTrendChart({
  ...
  const comparison = data.map((point) => point.comparison);
  const revenuePath = buildLinePath(revenue, width, height, 70, 30);
  const comparisonPath = buildLinePath(comparison, width, height, 70, 30);
  const maxValue = Math.max(1, ...revenue, ...comparison);
  ...
  return (
    <article className="admin-panel rounded-[30px] p-6 text-white">
      ...
      <h3 className="admin-display mt-2 text-4xl font-semibold">Processed Revenue vs Prior Period</h3>
      <p className="mt-2 text-sm text-pondo-text-secondary">
        Live processed revenue from reconciled transactions compared against {comparisonLabel.toLowerCase()}.
      </p>
      <svg viewBox={`0 0 ${width} ${height}`} className="mt-6 w-full">
        ...
        <path d={comparisonPath} fill="none" stroke="#9dc2f2" strokeWidth="3" strokeDasharray="7 5" />
        <path d={revenuePath} fill="none" stroke="#f5b642" strokeWidth="4" />
        {data.map((point, index) => {
          const x = 70 + (630 * index) / Math.max(data.length - 1, 1);
          const y = 30 + (1 - point.revenue / maxValue) * 190;
          return <circle key={point.label} cx={x} cy={y} r="8" fill="#f5b642" />;
        })}
      </svg>
      <div className="mt-4 flex flex-wrap justify-center gap-5 text-sm">
        <span className="flex items-center gap-2 text-amber-200"><span className="h-2.5 w-2.5 rounded-full bg-amber-400" />{currentLabel}</span>
        <span className="flex items-center gap-2 text-sky-200"><span className="h-2.5 w-6 rounded-full border-t-2 border-dashed border-sky-200" />{comparisonLabel}</span>
        {data.map((point) => (
          <span key={point.label} className="text-pondo-text-secondary">
            {point.label}: {formatCompact(point.revenue)}
          </span>
        ))}
      </div>
    </article>
