  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#e9f1ff_0%,#f7faff_34%,#edf4ff_100%)] text-[var(--pondo-navy-900)]">
      ...
      <div className="mx-auto max-w-3xl px-6 py-10">
        ...
        {!auth ? (
          <div className="mt-6 rounded-2xl border border-[var(--pondo-line)] bg-white p-6">
            <div className="text-sm text-slate-600">Sign in to view your confirmation.</div>
            <button onClick={quickLogin} disabled={busy} className="mt-4 rounded-xl bg-[var(--pondo-orange-500)] px-4 py-2 text-sm font-semibold text-white hover:bg-[var(--pondo-orange-400)] disabled:opacity-50">
          ...
          <div className="mt-6 grid gap-4">
            <div className="rounded-2xl border border-[var(--pondo-line)] bg-white p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-1 text-xs text-slate-500">
                    Status: <span className="font-semibold text-[var(--pondo-navy-800)]">{tx.status}</span> • Gateway:{" "}
                    <span className="font-semibold text-[var(--pondo-navy-800)]">{tx.gateway}</span>
                  </div>
                </div>
              ...
              <div className="mt-4 break-all rounded-xl border border-[var(--pondo-line)] bg-[var(--pondo-surface-soft)] p-3 font-mono text-[11px] text-slate-600">{tx.qr_payload}</div>
            </div>

            <div className="flex items-center justify-between">
              <Link href="/PondoDemo/shop" className="text-sm font-semibold text-[var(--pondo-navy-800)]/80 hover:text-white hover:underline">
                Continue shopping
              </Link>
              <Link href="/PondoDemo/sponsor" className="rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-[var(--pondo-navy-800)] hover:bg-white/10">
                View sponsor dashboard
              </Link>
            </div>
          </div>
        ) : (
          <div className="mt-6 rounded-2xl border border-[var(--pondo-line)] bg-white p-6 text-sm text-slate-600">Loading…</div>
        )}
      </div>
    </div>
  );
