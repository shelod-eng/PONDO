    ...
    <div className="min-h-screen bg-[#f1f3f7] text-pondo-navy-900">
      ...
      <div className="mx-auto max-w-[1240px] px-4 py-6">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href="/PondoDemo/checkout" className="rounded-xl bg-pondo-orange-500 px-6 py-3 text-sm font-bold text-white hover:bg-pondo-orange-400">
            Checkout
          </Link>
        </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-[2fr_1fr]">
          <section className="rounded-2xl border border-pondo-line bg-white p-5 shadow-sm">
            {lines.length === 0 ? (
              <div className="rounded-xl border border-dashed border-pondo-line bg-[#fafcff] p-8 text-center">
                ...
                <Link href="/PondoDemo/shop" className="mt-4 inline-flex rounded-lg bg-pondo-navy-900 px-4 py-2 text-sm font-semibold text-white hover:bg-pondo-navy-800">
                  Back to Shop
                </Link>
              </div>
            ) : (
              <div className="space-y-4">
                {lines.map((l) => {
                  ...
                  return (
                    <article key={l.product.id} className="grid items-center gap-3 rounded-xl border border-pondo-line bg-[#fbfdff] p-3 sm:grid-cols-[120px_1fr_auto_auto]">
          ...
          <aside className="rounded-2xl border border-pondo-line bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="text-right text-3xl font-black text-pondo-navy-900">{money(total)}</div>
            </div>
        ...
        {recommended.length > 0 ? (
          <section className="mt-6 rounded-2xl border border-pondo-line bg-white p-5 shadow-sm">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              {recommended.map((p) => (
                <div key={p.id} className="rounded-xl border border-pondo-line bg-[#fbfdff] p-3">
